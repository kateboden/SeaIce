...
for t in range(start, Nt):
    ...
    if( (t-first_assimilation)%5 ):
        ...
        #print('a0 prior', a0_prior)
        #print('a0_posterior', a0_posterior)
        # Step #2: Update all areas & volumes based on the new open water fraction
        #          Need to update this to handle NaNs
        scaling_factors = np.nan_to_num((1. - a0_posterior) / (1. - a0_prior), nan=1.0)
        scaling_factors[np.isinf(scaling_factors)] = 1. # Can't create ice from open water
        aicen_ens_posterior = copy.deepcopy(aicen_ens_prior) * scaling_factors[:,np.newaxis]
        vicen_ens_posterior = copy.deepcopy(vicen_ens_prior) * scaling_factors[:,np.newaxis]
